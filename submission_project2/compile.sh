...
# compile.sh
# Usage: 
#   ./compile.sh         - Clean and compile all Java files
#   ./compile.sh --clean - Only remove .class files (no compilation)

# Check if clean-only mode
CLEAN_ONLY=false
if [ "$1" == "--clean" ]; then
    CLEAN_ONLY=true
    echo "====================================="
    echo "Cleaning Java Project (no compile)"
    echo "====================================="
    echo ""
else
    echo "====================================="
